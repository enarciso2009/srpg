...
class FraudAlertSerializer(serializers.ModelSerializer):
    ...
    employee_matricula = serializers.CharField(source='work_shift.employee.matricula', read_only=True)
    shift_id = serializers.IntegerField(source='work_shift.id', read_only=True)

    class Meta:
        model = FraudAlert
        fields = ['id', 'employee_name', 'employee_matricula', 'user_email', 'shift_id', 'fraud_type', 'severity',
                  'score', 'description', 'created_at', 'resolved',]
